...
$('#btnSubmit').off("click").on("click", function () {
    const rememberUserCode = $("#rememberUserCode").parent().hasClass("checked");
    const rememberMe = $("#rememberMe").parent().hasClass("checked");
...
//检测用户输入数据
function loginFun() {
    const loginName = $.trim($('#loginName').val());
    ...
    if (loginName.length !== 0 && password.length !== 0) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if(res) {
                    const loginInfoTip = res.data.msgTip;
